...
class RuntimeTests(unittest.TestCase):
    def test_app_root_uses_executable_folder_when_frozen(self) -> None:
        with TemporaryDirectory() as temp_dir:
            portable_root = Path(temp_dir) / "Portable W-MIDI"
            executable = portable_root / "W-MIDI.exe"
            ...
            with (
                ...
                self.assertEqual(portable_root.resolve(), runtime.app_root())

    def test_app_root_uses_release_folder_for_macos_app_bundle(self) -> None:
        with TemporaryDirectory() as temp_dir:
            release_root = Path(temp_dir) / "W-MIDI-v1.2.0-macOS-arm64"
            executable = release_root / "W-MIDI.app" / "Contents" / "MacOS" / "W-MIDI"
            ...
            with (
                ...
                self.assertEqual(release_root.resolve(), runtime.app_root())

    def test_bridge_prefix_uses_frozen_executable_cli_mode(self) -> None:
        with TemporaryDirectory() as temp_dir:
            executable = Path(temp_dir) / "Portable W-MIDI" / "W-MIDI.exe"
            executable.parent.mkdir()
            executable.touch()
