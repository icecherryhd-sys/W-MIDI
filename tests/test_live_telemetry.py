...
class LiveTelemetryTests(unittest.TestCase):
    def test_bridge_emits_exact_led_frame_preview_when_enabled(self) -> None:
        ...
        bridge.leds[:2] = [(1, 31, 63), (255, 120, 4)]
        output = io.StringIO()

        class FakeSocket:
            def sendto(self, _payload, _address) -> None:
                pass

            def close(self) -> None:
                pass

        original_socket = bridge.sock
        bridge.sock = FakeSocket()
        try:
            with redirect_stdout(output):
                bridge.send_frame()

            self.assertIn("LED_FRAME rgb=011f3fff7804", output.getvalue())
        finally:
