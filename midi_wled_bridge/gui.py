...
class BridgeGuiApp:
    ...
    def _get_virtual_midi_manager(self) -> VirtualMidiPortManager:
        ...
        return self._virtual_midi_manager

    def _create_midi_port_clicked(self) -> None:
        window = tk.Toplevel(self.root)
        window.title("Create New Midi Port")
        window.geometry("380x150")
        window.resizable(False, False)
        window.configure(bg=self._t["panel"])
        window.columnconfigure(0, weight=1)
        ...
        name = tk.StringVar(value="W-MIDI")
        ...
        footer = tk.Frame(window, bg=self._t["panel"])
        ...
        def add_port() -> None:
            try:
                port_name = self._get_virtual_midi_manager().create_port(name.get())
            except (ValueError, VirtualMidiError) as exc:
                messagebox.showerror("Could not create MIDI port", str(exc), parent=window)
                return
            self._reload_ports()
            self._midi_combo.set(port_name)
            self._append_log_line(f"Created temporary MIDI port: {port_name}")
            window.destroy()

        self._button(footer, "Cancel", window.destroy).grid(row=0, column=0, sticky="ew", padx=(0, 6))
        self._button(footer, "Add", add_port, bg=self._t["cyan"], hover=self._t["cyan_hover"], fg="#061019").grid(
            row=0, column=1, sticky="ew", padx=(6, 0)
        )
        window.bind("<Return>", lambda _: add_port())
        window.bind("<Escape>", lambda _: window.destroy())

    def _find_wled_clicked(self) -> None:
        if self._discovery_window is not None and self._discovery_window.winfo_exists():
            self._discovery_window.lift()
            self._discovery_window.focus_force()
            return

        window = tk.Toplevel(self.root)
        window.title("Find WLED Devices")
        ...
        self._discovery_status = tk.StringVar(value="Searching local network...")
        tk.Label(
            window,
            textvariable=self._discovery_status,
            bg=self._t["panel"],
            fg=self._t["text_dim"],
            font=self._font_small,
            anchor="w",
        ...
        self._discovery_list = tk.Listbox(
            body,
            bg=self._t["log_bg"],
            fg=self._t["text"],
            selectbackground=self._t["cyan_dark"],
        ...
        scroll = ttk.Scrollbar(body, command=self._discovery_list.yview, style="Bridge.Vertical.TScrollbar")
        self._discovery_list.configure(yscrollcommand=scroll.set)
        self._discovery_list.grid(row=0, column=0, sticky="nsew")
        scroll.grid(row=0, column=1, sticky="ns")
        self._discovery_list.bind("<Double-Button-1>", lambda _: self._select_discovered_wled())

        footer = tk.Frame(window, bg=self._t["panel"])
        footer.grid(row=2, column=0, sticky="ew", padx=12, pady=(0, 12))
        footer.columnconfigure(0, weight=1)
        self._button(footer, "Use Selected IP", self._select_discovered_wled).grid(row=0, column=0, sticky="ew", padx=(0, 8))
        self._button(footer, "Search Again", self._run_wled_discovery).grid(row=0, column=1, sticky="ew")

        def on_close() -> None:
            self._discovery_window = None
            self._discovery_list = None
        ...
        self._run_wled_discovery()

    def _run_wled_discovery(self) -> None:
        if self._discovery_list is not None:
            self._discovery_list.delete(0, "end")
        if self._discovery_status is not None:
            self._discovery_status.set("Searching local network...")
        self._discovery_devices = []
        self._append_log_line("Searching for WLED devices on the local network...")

        def worker() -> None:
            try:
                devices = discover_wled_devices()
            except Exception as exc:
                self.root.after(0, lambda: self._finish_wled_discovery_error(str(exc)))
                return
            self.root.after(0, lambda: self._finish_wled_discovery(devices))

        threading.Thread(target=worker, daemon=True).start()

    def _finish_wled_discovery(self, devices: list[WledDevice]) -> None:
        self._discovery_devices = devices
        if self._discovery_list is not None:
            self._discovery_list.delete(0, "end")
            for device in devices:
                self._discovery_list.insert("end", f"{device.name} - {device.ip}")
        count = len(devices)
        message = f"Found {count} WLED device{'s' if count != 1 else ''}."
        if count == 0:
            message = "No WLED devices found. Check that your ESP32 is powered on and connected."
        if self._discovery_status is not None:
            self._discovery_status.set(message)
        self._append_log_line(message)

    def _finish_wled_discovery_error(self, detail: str) -> None:
        message = "WLED search failed."
        if self._discovery_status is not None:
            self._discovery_status.set(message)
        self._append_log_line(f"{message} {detail}")

    def _select_discovered_wled(self) -> None:
        if self._discovery_list is None:
            return
        selection = self._discovery_list.curselection()
        if not selection:
            if self._discovery_status is not None:
                self._discovery_status.set("Select a WLED device first.")
            return
        device = self._discovery_devices[int(selection[0])]
        self._wled_ip.set(device.ip)
        self._append_log_line(f"Selected WLED device: {device.name} ({device.ip})")
        if self._discovery_status is not None:
