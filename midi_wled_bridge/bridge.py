...
class MidiToWledBridge:
    ...
    def run(self) -> None:
        ...
        with mido.open_input(self.cfg.midi_port) as in_port:
            self._run_message_loop(in_port.iter_pending)

    def _run_virtual_midi(self) -> None:
        assert self.cfg.virtual_midi_udp_port is not None
        print(f"Connecting W-MIDI virtual input: {self.cfg.midi_port}")
        parser = mido.Parser()
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
            sock.bind(("127.0.0.1", self.cfg.virtual_midi_udp_port))
            sock.setblocking(False)
            self._run_message_loop(
                lambda: read_virtual_midi_messages(sock, parser, self.cfg.midi_read_burst)
            )
