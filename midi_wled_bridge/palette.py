...
def scale_palette_to_full(
    ...
    return {
        velocity: tuple(min(255, round(channel * 255 / 63)) for channel in rgb)  # type: ignore[misc]
        for velocity, rgb in palette.items()
    }


def _parse_zero_based_space_palette(lines: List[str]) -> Dict[int, Tuple[int, int, int]]:
    mapping: Dict[int, Tuple[int, int, int]] = {}
    pattern = re.compile(r"^(\d+)\s*,\s*(\d+)\s+(\d+)\s+(\d+)\s*;?$")
    for line in lines:
        match = pattern.fullmatch(line)
        if match is None:
            raise argparse.ArgumentTypeError(
                f"Invalid palette entry '{line}'. Use velocity, R G B;"
            )
        velocity, red, green, blue = (int(value) for value in match.groups())
        if velocity < 0 or velocity > 127:
            raise argparse.ArgumentTypeError(f"Velocity must be 0..127, got {velocity}.")
        rgb = red, green, blue
        if any(channel < 0 or channel > 255 for channel in rgb):
            raise argparse.ArgumentTypeError("RGB values must be between 0 and 255.")
        mapping[velocity] = rgb
    if not mapping:
        raise argparse.ArgumentTypeError("Velocity palette has no valid entries.")
    return mapping
